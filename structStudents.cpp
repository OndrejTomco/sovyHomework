...
int i;
   ...
   printf("\nHighest salary:%s, %d", maxName, max);
   
   printf("\nAll students from Kosice or Presov: ");
   char keString[10];
   char poString[10];
   strcpy(keString, "Kosice");
   strcpy(poString, "Presov");
   for(i=0;i<20;i++) {
   		if(strcmp(keString, student[i].address.city ) == 0 || strcmp(poString, student[i].address.city) == 0){
   ...
   for(i=0;i<20;i++) {
   	...
   	if(student[i].salary >999 && student[i].salary<1300) {
   		printf("%s ", student[i].name);
	   }
   	
	count += student[i].salary;
	if(student[i].age < 30) {
   		countStudents++;
   		count30 += student[i].salary;
	   }
	   
	if(strcmp(keString, student[i].address.city ) != 0) {
		notFromKe++;
	}
	
	if(student[i].age>oldest){
	...
	avg = count/20;
	avg30 = count30/countStudents;
    printf("\nAvarage salary: %1.2f", avg);
    printf("\nAvarage salary of students under 30: %1.2f", avg30);
    printf("\nNumber of students who are not from KE: %d", notFromKe++);
    printf("\nOldest: %s %d", OldestName, oldest);
    printf("\nNumber of students with name longer than 7: %d",moreThen7 );
...
printf("\nList of students with ascending salaries:");
for(i=0;i<20;i++) {
